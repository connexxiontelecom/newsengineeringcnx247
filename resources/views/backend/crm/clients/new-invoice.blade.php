				...
				<div class="card-block">
						...
						<div class="col-md-4 col-xs-12 invoice-client-info">
							...
							<select name="clientId" id="clientId" class="form-control">
								...
								@foreach($clients as $c)
									<option value="{{$c->id}}" data-account="{{$c->glcode ?? ''}}">{{$c->company_name ?? $c->first_name }}</option>
								@endforeach
							</select>
							<div class="form-group client-account">
					...
					<div class="row text-center">
						<div class="col-sm-12 invoice-btn-group text-center">
							...
							<a href="{{url()->previous()}}" class="btn btn-danger btn-mini waves-effect m-b-10 btn-sm waves-light">Cancel</a>
							<button type="submit" class="btn btn-primary btn-mini btn-print-invoice m-b-10 btn-sm waves-effect waves-light m-r-20"> <i class="ti-control-shuffle"></i> Submit</button>
						</div>
					</div>
				</div>
			</div>
	...
	<script>
		...
		$(document).ready(function(){
			...
			$(document).on('change', '#clientId', function (e) {
					var glcode = $(this).find(':selected').data('account');
					if(glcode == ''){
						$('.client-account').show();
					}else{
						$('.client-account').hide();
